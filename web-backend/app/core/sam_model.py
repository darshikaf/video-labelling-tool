...
class SAMModel:
    ...
    def _run_segmentation(self, image: np.ndarray) -> Optional[List[Dict]]:
        ...
        try:
            results = self.model.predict(
                source=image, points=self.points, labels=self.labels, show=False
            )
    ...
    def predict(self, image, prompt_type="point", points=None, boxes=None, masks=None):
        ...
        if self.model is None:
            ...
            if prompt_type == "point" and points:
                ...
                for x1, y1, x2, y2 in boxes:
                    cv2.rectangle(
                        result_mask, (int(x1), int(y1)), (int(x2), int(y2)), 1, -1
                    )
            else:
        ...
        if prompt_type == "point" and points:
            ...
            for x, y, is_positive in points:
                logger.info(
                    f"SAMModel: Adding point ({x}, {y}) with label {1 if is_positive else 0}"
                )
            ...
            if (
                ...
                logger.info(
                    f"SAMModel: Got {len(results[0].masks.data)} masks from SAM"
                )
                for i, mask in enumerate(results[0].masks.data):
                    mask_np = mask.cpu().numpy().astype("uint8")
                    logger.info(
                        f"SAMModel: Processing mask {i}, shape: {mask_np.shape}, unique values: {np.unique(mask_np)}"
                    )
            ...
            logger.info("SAMModel: Processing box prompts with actual SAM model")
            # For box prompts, use the SAM2 bboxes parameter
            for x1, y1, x2, y2 in boxes:
                logger.info(f"SAMModel: Processing box ({x1}, {y1}, {x2}, {y2})")
                try:
                    # SAM2 expects bboxes in [x1, y1, x2, y2] format
                    ...
                    if (
                        ...
                        logger.info(
                            f"SAMModel: Got {len(results[0].masks.data)} masks from SAM for box"
                        )
                        for mask in results[0].masks.data:
                            mask_np = mask.cpu().numpy().astype("uint8")
                            result_mask = np.logical_or(result_mask, mask_np).astype(
                                np.uint8
                            )
                    else:
                        logger.warning(
                            "SAMModel: No masks returned from SAM model for box, using fallback"
                        )
                    ...
                    logger.error(
                        f"SAMModel: Box prediction failed: {e}, using fallback"
                    )
                    # Fallback to simple rectangle mask
                    cv2.rectangle(
                        result_mask, (int(x1), int(y1)), (int(x2), int(y2)), 1, -1
        ...
        logger.info(
            f"SAMModel: Final result mask shape: {result_mask.shape}, non-zero pixels: {np.count_nonzero(result_mask)}"
        )
        return result_mask
    ...
    def resize_frame(self, frame, target_width=640, target_height=480):
        ...
        logger.info(
            f"SAMModel: Resized frame from {w}x{h} to {target_width}x{target_height}"
        )
        logger.info(
            f"SAMModel: Actual image size: {new_w}x{new_h}, offset: ({x_offset}, {y_offset})"
        )
    ...
    def predict_from_base64(
        ...
        try:
            ...
            logger.info(
                f"SAMModel: Converted points (640x480 coordinates): {point_list}"
            )
            ...
            logger.info(
                f"SAMModel: Mask coverage: {np.count_nonzero(mask) / mask.size * 100:.2f}%"
            )
            ...
            if mask.shape != (480, 640):
                ...
                logger.error(
                    "This indicates an issue with the SAM model or prediction process"
                )
            ...
            logger.info(
                f"SAMModel: Final mask image dimensions: {mask_image.size} (should be 640x480)"
            )
            if mask_image.size != (640, 480):
                logger.error(
                    f"Final mask dimensions incorrect: {mask_image.size}, expected (640, 480)"
                )
